  ...
  window.dialog = {
    ...
    prompt: function (b) {
      ...
      var b = a.extend(!0, {}, dialog.defaultParams, b),
        c = dialog.generateRandomId(),
        d = '';
      for (var e in b.input) d += ' ' + e + '="' + b.input[e] + '" ';
    ...
    showDialog: function () {
      ...
      'absolute' === b.attr('data-dialog-position')
        ? (dialog.holder.removeClass('dialog-fixed'),
          dialog.holder.css('top', a(window).scrollTop()))
        ...
        setTimeout(function () {
          b.bind(dialog.transitionEnd, function (c) {
            a(c.target).is(this) &&
              (b.unbind(dialog.transitionEnd),
              dialog.focusElement(b.find('input')[0], !0));
          }).addClass('dialog-visible');
        }, 1),
        a('html').addClass('dialogIsVisible');
    },
    injectDialog: function () {
      0 === a('.dialog-alert:visible').length
        ? dialog.showDialog()
        : a('.dialog-alert:last').hide(),
        dialog.overlay.addClass('dialog-visible');
    },
    focusElement: function (b, c) {
      b &&
        (a(b).one('blur.dialog', function () {
    ...
    close: function () {
      ...
      b.addClass('dialog-closing')
        .bind(dialog.transitionEnd, function (c) {
          a(c.target).is(this) &&
            (b.unbind(dialog.transitionEnd),
            b.remove(),
            a('html').removeClass('dialogIsVisible'),
            0 === a('.dialog-alert').length
              ? dialog.overlay
                  .addClass('dialog-closing')
                  .bind(dialog.transitionEnd, function (b) {
                    a(b.target).is(this) &&
    ...
    bindDialogGlobalEvents: function () {
      dialog.holder.add(dialog.overlay).bind('click.dialog', function (b) {
        a(b.target).closest('.dialog-alert').is('.dialog-alert') ||
          a('.dialog-close:visible').trigger('click');
      }),
        a(document).on(
          'click.dialog',
        ...
        a(document).bind('keydown.dialog', function (b) {
          ...
          if (0 !== c.length)
            return 13 == b.keyCode
              ? (c.find('.dialog-confirm').trigger('click'), !1)
              : void 0;
        }),
...
$('#confirm').click(function () {
  dialog.confirm({
    ...
    message:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce consectetur lacus at gravida luctus. Duis vitae magna tellus. In risus lorem, mollis vel nisi vitae, suscipit aliquet tortor.',
    cancel: 'Cancel',
    button: 'Accept',
    required: true,
